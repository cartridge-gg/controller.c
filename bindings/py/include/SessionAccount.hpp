...
namespace capi {
    ...
    typedef struct SessionAccount_create_from_subscribe_create_session_result {union {diplomat::capi::SessionAccount* ok; diplomat::capi::ControllerError* err;}; bool is_ok;} SessionAccount_create_from_subscribe_create_session_result;
    SessionAccount_create_from_subscribe_create_session_result SessionAccount_create_from_subscribe_create_session(const diplomat::capi::DiplomatSigner* signer, const diplomat::capi::DiplomatSessionPolicies* policies, diplomat::capi::DiplomatStringView rpc_url, diplomat::capi::DiplomatStringView cartridge_api_url);
    ...
    SessionAccount_new_as_registered_result SessionAccount_new_as_registered(diplomat::capi::DiplomatStringView rpc_url, const diplomat::capi::DiplomatSigner* signer, const diplomat::capi::DiplomatFelt* address, const diplomat::capi::DiplomatFelt* owner_guid, const diplomat::capi::DiplomatFelt* chain_id, const diplomat::capi::DiplomatSessionPolicies* policies, uint64_t session_expiration);
...
inline diplomat::result<diplomat::result<std::unique_ptr<SessionAccount>, std::unique_ptr<ControllerError>>, diplomat::Utf8Error> SessionAccount::create_from_subscribe_create_session(const DiplomatSigner& signer, const DiplomatSessionPolicies& policies, std::string_view rpc_url, std::string_view cartridge_api_url) {
    if (!diplomat::capi::diplomat_is_str(cartridge_api_url.data(), cartridge_api_url.size())) {
    return diplomat::Err<diplomat::Utf8Error>();
  }
    auto result = diplomat::capi::SessionAccount_create_from_subscribe_create_session(signer.AsFFI(),
        policies.AsFFI(),
        {rpc_url.data(), rpc_url.size()},
        {cartridge_api_url.data(), cartridge_api_url.size()});
    return diplomat::Ok<diplomat::result<std::unique_ptr<SessionAccount>, std::unique_ptr<ControllerError>>>(result.is_ok ? diplomat::result<std::unique_ptr<SessionAccount>, std::unique_ptr<ControllerError>>(diplomat::Ok<std::unique_ptr<SessionAccount>>(std::unique_ptr<SessionAccount>(SessionAccount::FromFFI(result.ok)))) : diplomat::result<std::unique_ptr<SessionAccount>, std::unique_ptr<ControllerError>>(diplomat::Err<std::unique_ptr<ControllerError>>(std::unique_ptr<ControllerError>(ControllerError::FromFFI(result.err)))));
}

inline diplomat::result<std::unique_ptr<SessionAccount>, std::unique_ptr<ControllerError>> SessionAccount::new_as_registered(std::string_view rpc_url, const DiplomatSigner& signer, const DiplomatFelt& address, const DiplomatFelt& owner_guid, const DiplomatFelt& chain_id, const DiplomatSessionPolicies& policies, uint64_t session_expiration) {
