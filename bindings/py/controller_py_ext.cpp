...
NB_MODULE(controller_py, controller_py_mod)
{
	{
		nb::class_<_Dummy> dummy(controller_py_mod, "__dummy__");
		nb_tp_dealloc = (void (*)(void *))nb::type_get_slot(dummy, Py_tp_dealloc);
	}

    nb::class_<std::monostate>(controller_py_mod, "monostate")
    ...
    // Module declarations
    // Add bindings
    add_CONTROLLERS_binding(controller_py_mod);
    add_Controller_binding(controller_py_mod);
    add_ControllerInner_binding(controller_py_mod);
    add_ControllerError_binding(controller_py_mod);
    add_DiplomatCall_binding(controller_py_mod);
    add_DiplomatCallList_binding(controller_py_mod);
    add_DiplomatFelt_binding(controller_py_mod);
    add_SessionAccount_binding(controller_py_mod);
    add_SessionAccountInner_binding(controller_py_mod);
    add_DiplomatPolicies_binding(controller_py_mod);
    add_DiplomatOwner_binding(controller_py_mod);
    add_DiplomatSigner_binding(controller_py_mod);
    add_ResponseDataOut_binding(controller_py_mod);
    add_SubscribeCreateSessionResponse_binding(controller_py_mod);
    add_Utils_binding(controller_py_mod);
    add_SignerType_binding(controller_py_mod);
    add_Version_binding(controller_py_mod);
    add_OwnerType_binding(controller_py_mod);
