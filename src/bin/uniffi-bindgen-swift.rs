...
fn main() {
    ...
    if args.len() > 1 && (args[1] == "--help" || args[1] == "-h") {
        ...
        eprintln!(
            "  {} target/release/libcontroller_uniffi.dylib bindings/swift --headers --modulemap",
            args[0]
        );
        eprintln!();
        process::exit(0);
    ...
    let generate_swift_sources = has_swift_sources || (!has_headers && !has_modulemap) || (has_headers);
    let generate_headers = has_headers || (!has_swift_sources && !has_modulemap) || has_swift_sources;
    ...
    let options = SwiftBindingsOptions {
        ...
        module_name: Some("ControllerSDK".to_string()),
