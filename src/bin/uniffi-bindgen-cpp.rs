...
fn main() {
    ...
    if args.len() > 1 && (args[1] == "--help" || args[1] == "-h") {
        eprintln!("UniFFI C++ Binding Generator for Controller");
        eprintln!();
        ...
        eprintln!("  {} src/controller.udl ../../bindings/cpp", args[0]);
        eprintln!();
    ...
    // Default paths
    let default_udl = "src/controller.udl";
    ...
    match cmd.output() {
        Ok(output) => {
            if output.status.success() {
                ...
                println!("  - controller.hpp (C++ header)");
                println!("  - controller.cpp (C++ implementation)");
                println!("  - controller_scaffolding.hpp (FFI scaffolding)");
                ...
                println!("\nTo use the bindings:");
                println!("  1. Include controller.hpp in your C++ project");
                println!("  2. Link against the controller-uniffi library");
